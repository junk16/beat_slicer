...
typedef struct
{
  ...
  int bits; /* 量子化精度 */
  int length; /* 音データの長さ */
  double *buffer; /* 音データ（Lチャンネル） */
} PCM;



void read_wave(PCM *pcm, char *file_name)
{
  FILE *fp;
  int n;
  ...
  pcm->buffer = calloc(pcm->length, sizeof(double));
...
printf("size : %d\n", data_chunk_size / (fmt_channel*(fmt_bits_per_sample/8)));
printf("chunk size : %d\n", pcm->length);
  for (n = 0; n < data_chunk_size / 2; n++)
  {
    fread(&data, 2, 1, fp);
//    printf("%d:%f\n", n, (double)data  / 32768.0 );

    pcm->buffer[n] = (double)data / 32768.0;
...
void monot_to_stereo_wave_write(STEREO_PCM *pcm, char *file_name)
{
  FILE *fp;
  int n;
  char riff_chunk_ID[4];
